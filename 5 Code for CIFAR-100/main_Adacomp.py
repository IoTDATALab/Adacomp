...
def main():
    ...
    lr_list = []
    lr = args.lr
    for epoch in range(args.start_epoch, int(0.6 * args.epochs)):

        # train for one epoch
        lr, lr_list_inner = train(trainloader, model, criterion, optimizer, epoch, losses_neib, lr)
        lr_list.append(lr_list_inner)

        # evaluate on test set
        prec, loss = validate(testloader, model, criterion)
    ...
    optimizer = optim.SGD(model.parameters(), lr=lr_second_phase)
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.7)

    for epoch in range(args.start_epoch, int(0.4 * args.epochs)):
        train1(trainloader, model, criterion, optimizer, epoch+int(0.6 * args.epochs))
        scheduler.step()
        prec, loss = validate(testloader, model, criterion)
        prec_list.append(prec)
        loss_list.append(loss)
    ...
    with open('./result_adacomp1004.txt', 'a+') as fl:
        fl.write('\n Optimizer is Adacomp, LR is {}, \n test loss is {},'
                 '\n test acc is {} \n \n'.format(args.lr, loss_list, prec_list))
...
def train(trainloader, model, criterion, optimizer, epoch, losses_neib, lr):
    ...
    for i, (input, target) in enumerate(trainloader):
        ...
        if i % args.print_freq == 0:
            ...
            lr_list_inner.append(lr.data.item())
            print('Epoch: [{0}][{1}/{2}]\t'
    ...
    return lr.data.item(), lr_list_inner


def train1(trainloader, model, criterion, optimizer, epoch):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
