...
parser.add_argument('--beta', default=5.0, type=float, help='increment and decrement tradeoff')
...
label = trainloader.dataset.classes
...
def train(epoch, lr, optimizer, losses_neib):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if batch_idx % 20 == 0:
            ...
            amplify21 = 1/4 - (torch.atan(torch.as_tensor(lr)))/(args.beta*3.14)
...
def test(epoch):
    ...
    # Save checkpoint.
            ...
            confusion.update(predicted, targets)
    # confusion.plot()
...
for epoch in range(start_epoch, start_epoch + int(fraction*total_epochs)):
    ...
    acc, precision, recall, f1_score = test(epoch)
    # test_loss.append(a)
...
with open('./results_1022.txt', 'a+') as fl:
    fl.write('Optimizer is AdaCompen, LR is {}, \n '
             ' test acc is{} \n test pre is {} \n test reca is {} \n test f1 is {}\n time is {} \n\n'.
             format(args.lr, test_acc, test_precision, test_recall,
                    test_f1, end - begin))
